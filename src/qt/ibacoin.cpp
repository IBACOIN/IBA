...
#include "qt/ibacoin/splash.h"
#include "qt/ibacoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("iba coin", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ibacoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ibacoin.qrc)
...
/** Class encapsulating IBA Coin startup and shutdown.
...
/** Main IBACoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (IBACoinGUI)
...
{
    window = new IBACoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &IBACoinGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &IBACoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(IBACoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(IBACoinGUI::DEFAULT_WALLET);
        ...
        // IBACoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &IBACoinGUI::handlePaymentRequest);
        connect(window, &IBACoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. IBA Coin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(ibacoin_locale);
    Q_INIT_RESOURCE(ibacoin);
    ...
    /// 6. Determine availability of data directory and parse ibacoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("IBA Coin"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("IBA Coin"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // ibacoin: links repeatedly have their payment requests routed to this process:
