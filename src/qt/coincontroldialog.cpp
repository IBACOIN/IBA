...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select IBA Outputs to Spend" :
            "Select Shielded IBA to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::IBA;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show IBACoin address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
