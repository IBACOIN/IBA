...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::IBA);
    ...
    if (settings.contains("nAnonymizeibacoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizeibacoinamount", settings.value("nAnonymizeibacoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ibacoin.cpp during shotdown to also remove the window positions
