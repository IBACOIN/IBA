...
#include "qt/ibacoin/forms/ui_addresseswidget.h"
#include "qt/ibacoin/addresslabelrow.h"
#include "qt/ibacoin/addnewaddressdialog.h"
#include "qt/ibacoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(IBACoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto ibaAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(ibaAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(ibaAdd);
        if (walletModel->updateAddressBookLabels(ibaAdd, label.toUtf8().constData(),
