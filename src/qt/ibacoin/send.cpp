...
#include "qt/ibacoin/send.h"
#include "qt/ibacoin/forms/ui_send.h"
#include "qt/ibacoin/addnewcontactdialog.h"
#include "qt/ibacoin/qtutils.h"
#include "qt/ibacoin/sendchangeaddressdialog.h"
#include "qt/ibacoin/optionbutton.h"
#include "qt/ibacoin/sendconfirmdialog.h"
#include "qt/ibacoin/guitransactionsutils.h"
#include "qt/ibacoin/loadingdialog.h"
...
SendWidget::SendWidget(IBACoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-iba");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onIBASelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onIBASelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent IBAs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield IBAs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent IBAs to shield."));
    }
}
...
void SendWidget::onIBASelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto ibaAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(ibaAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(ibaAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ibaAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
