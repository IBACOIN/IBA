...
#include "qt/ibacoin/settings/settingsfaqwidget.h"
#include "qt/ibacoin/qtutils.h"
#include "qt/ibacoin/settings/forms/ui_settingsfaqwidget.h"
#include <QMetaObject>
#include <QScrollBar>

SettingsFaqWidget::SettingsFaqWidget(IBACoinGUI* parent) : QDialog(parent),
                                                                           ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({ui->labelSubtitle_Intro,
                       ui->labelSubtitle_UnspendableIBA,
    ...
    setCssProperty({ui->labelContent_Intro,
                       ui->labelContent_UnspendableIBA,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("IBA Coin is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. IBA Coin incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("IBA Coin utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of IBA Coin to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableIBAContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received IBA Coin requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your IBA Coin wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableIBA->setText(unspendableIBAContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of IBA Coin with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("iba coin", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ui->labelContent_Stake->setText(stakeContent);

    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("For Support you can contact us on our Email: support@ibacoin.com")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node IBA Coin wallet with a "
               "requirement of 10,000 IBA secured collateral to provide extra services "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in IBA Coin Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the IBA Coin network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("10,000 IBA per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 10,000 IBA collateral "
    ...
    connect(ui->pushButton_UnspendableIBA, &QPushButton::clicked, [this]() { onFaqClicked(ui->widget_UnspendableIBA); });
    ...
    if (parent)
        connect(parent, &IBACoinGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
