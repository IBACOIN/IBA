...
#include "qt/ibacoin/forms/ui_snackbar.h"
#include "qt/ibacoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(IBACoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &IBACoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
