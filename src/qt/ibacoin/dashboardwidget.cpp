...
#include "qt/ibacoin/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/ibacoin/forms/ui_dashboardwidget.h"
#include "qt/ibacoin/qtutils.h"
#include "qt/ibacoin/sendconfirmdialog.h"
#include "qt/ibacoin/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(IBACoinGUI* parent) : PWidget(parent),
                                                                       ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareIba, "square-chart-iba");
    setCssProperty(ui->labelIba, "text-chart-iba");
    ...
    setCssProperty(ui->labelAmountIba, "text-stake-iba-disable");
    ...
    if (window)
        connect(window, &IBACoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("IBA")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair IBA
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            iba = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalIba += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesIba.append(iba);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(148, 118, 34));
    ...
    set0->append(chartData->valuesIba);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalIba > 0) {
        setCssProperty(ui->labelAmountIba, "text-stake-iba");
    } else {
        setCssProperty(ui->labelAmountIba, "text-stake-iba-disable");
    }
    forceUpdateStyle({ui->labelAmountIba});
    ui->labelAmountIba->setText(GUIUtil::formatBalance(chartData->totalIba, nDisplayUnit));
