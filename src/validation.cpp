...
#if defined(NDEBUG)
#error "IBACoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uiba) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("ibacoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> ibaInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                ibaInputs.push_back(stakeIn);
        }
        const bool hasIBAInputs = !ibaInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasIBAInputs) {
                    ...
                    for (const CTxIn& ibaIn : ibaInputs)
                        if(ibaIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasIBAInputs) {
                            for (const CTxIn& stakeIn : ibaInputs)
