...
CIbaStake* CIbaStake::NewIbaStake(const CTxIn& txin)
{
    ...
    return new CIbaStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CIbaStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CIbaStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CIbaStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CIbaStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CIbaStake::GetUniqueness() const
{
    //The unique identifier for a IBA stake is the outpoint
...
const CBlockIndex* CIbaStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CIbaStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CIbaStake::ContextCheck(int nHeight, uint32_t nTime)
{
