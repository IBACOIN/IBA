...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\IBACoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\IBACoin
// Mac: ~/Library/Application Support/IBACoin
// Unix: ~/.ibacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IBACoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IBACoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IBACoinParams
    // Mac: ~/Library/Application Support/IBACoinParams
    // Unix: ~/.ibacoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IBACoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "IBACoinParams";
#else
    // Unix
    return pathRet / ".ibacoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/ibacoin/sapling-spend.params";
        sapling_output = "/usr/share/ibacoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/ibacoin/sapling-spend.params";
            sapling_output = "/usr/local/share/ibacoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty ibacoin.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override ibacoin.conf
