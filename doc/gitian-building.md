...
*Setup instructions for a gitian build of IBA Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the IBACoin
...
to the IBACoin GitHub release page.
...
- [Building IBA Coin](#building-ibacoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ibacoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ibacoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ibacoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ibacoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ibacoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ibacoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ibacoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ibacoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ibacoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ibacoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ibacoin-osx-*-build.assert
```
