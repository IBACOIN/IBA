UNIX BUILD NOTES
====================
Some notes on how to build IBA Coin in Unix.

Note
---------------------
Always use absolute paths to configure and compile IBA Coin and the dependencies,
...
This will build ibacoin-qt as well, if the dependencies are met.
...
memory available when compiling IBA Coin. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~ibacoin/+archive/ibacoin).
 You can add the repository using the following command:

    sudo apt-get install software-properties-common
    sudo add-apt-repository ppa:ibacoin/ibacoin
    sudo apt-get update
...
To build IBA Coin without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build ibacoin-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a ibacoin-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip ibacoind" to strip the debug
...
Security
--------
To help make your IBA Coin installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, IBA Coin should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, IBA Coin may be compiled in
