...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/IBACoinParams"
    else
        PARAMS_DIR="$HOME/.ibacoin-params"
    fi
fi
...
Failed to fetch the IBACoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
IBACoin - fetch-params.sh

This script will fetch the IBACoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common IBACoin zkSNARK parameters. Note that it is
