...
class ReorgStakeTest(ibacoinTestFramework):
    ...
    def check_money_supply(self, expected_iba):
        # verify that nodes have the expected IBA supply
        iba_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(iba_supply, [DecimalAmt(expected_iba)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that IBA supply was properly updated after the reorgs
        self.log.info("Check IBA supply...")
